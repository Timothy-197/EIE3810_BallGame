...
{
	u32 index;
	EIE3810_TFTLCD_WrCmd(0x2A00); // set column begin (x)
	EIE3810_TFTLCD_WrData(0);
	EIE3810_TFTLCD_WrCmd(0x2A01);
	EIE3810_TFTLCD_WrData(0);
	
	EIE3810_TFTLCD_WrCmd(0x2A02); // set column END
	EIE3810_TFTLCD_WrData(479>>8);
	EIE3810_TFTLCD_WrCmd(0x2A03);
	EIE3810_TFTLCD_WrData(479&0xFF);
	
	EIE3810_TFTLCD_WrCmd(0x2B00); // set row begin (y)
	EIE3810_TFTLCD_WrData(0);
	EIE3810_TFTLCD_WrCmd(0x2B01);
	EIE3810_TFTLCD_WrData(0);
	
	EIE3810_TFTLCD_WrCmd(0x2B02); // set row end
	EIE3810_TFTLCD_WrData(799>>8);
	EIE3810_TFTLCD_WrCmd(0x2B03);
	EIE3810_TFTLCD_WrData(799&0xFF);
	
	EIE3810_TFTLCD_WrCmd(0x2C00); // color the screen
...
{
	u8 i, j, k;
	if (ASCII < 32 || ASCII == 127) return; // cannot print
	for (i=0; i<8; i++) // iterate the columns from the left 0~7
	{
		for (j=0; j<8; j++)
		{
			if ((asc2_1608[ASCII-32][2*i]>>(7-j)) & 0x1) // if the pixel should be colored
			{
		...
		for (k=8; k<16; k++)
		{
			if ((asc2_1608[ASCII-32][2*i+1]>>(15-k)) & 0x1) // if the pixel should be colored
			{
...
void EIE3810_TFTLCD_ShowCharLine(u16 x, u16 y, char* line, u16 color, u16 bgcolor)
{
	u8 i = 0;
	while (line[i] != 0x00)
	{
		EIE3810_TFTLCD_ShowChar(x+i*8, y, line[i], color, bgcolor);
		i++;
	}
}
...
void EIE3810_TFTLCD_ShowCharLine2412(u16 x, u16 y, char* line, u16 color, u16 bgcolor)
{
	u8 i = 0;
	while (line[i] != 0x00)
	{
		EIE3810_TFTLCD_ShowChar2412(x+i*12, y, line[i], color, bgcolor);
		i++;
	}
}
...
void EIE3810_TFTLCD_SevenSegment(u16 start_x, u16 start_y, u8 digit, u16 color)
{
	if (digit == 0)
	{
		EIE3810_TFTLCD_FillRectangle(start_x+10, 55, start_y, 10, color); // seg1
		EIE3810_TFTLCD_FillRectangle(start_x, 10, start_y+10, 55, color); // seg2
		EIE3810_TFTLCD_FillRectangle(start_x+65, 10, start_y+10, 55, color); // seg3
		EIE3810_TFTLCD_FillRectangle(start_x, 10, start_y+75, 55, color); // seg5
		EIE3810_TFTLCD_FillRectangle(start_x+65, 10, start_y+75, 55, color); // seg6
		EIE3810_TFTLCD_FillRectangle(start_x+10, 55, start_y+130, 10, color); // seg7
	}
	else if (digit == 1)
	{
		EIE3810_TFTLCD_FillRectangle(start_x+65, 10, start_y+10, 55, color); // seg3
	...
	else if (digit == 2)
	{
		EIE3810_TFTLCD_FillRectangle(start_x+10, 55, start_y, 10, color); // seg1
		EIE3810_TFTLCD_FillRectangle(start_x+65, 10, start_y+10, 55, color); // seg3
		EIE3810_TFTLCD_FillRectangle(start_x+10, 55, start_y+65, 10, color); // seg4
		EIE3810_TFTLCD_FillRectangle(start_x, 10, start_y+75, 55, color); // seg5
		EIE3810_TFTLCD_FillRectangle(start_x+10, 55, start_y+130, 10, color); // seg7
	}
	else if (digit == 3)
